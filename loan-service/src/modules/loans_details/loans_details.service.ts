...
export class LoansDetailsService {
  constructor(
    ...
    console.log('Registered Mongoose models:', this.connection.modelNames());
    // Manual fallback for MissingSchemaError
    if (!this.connection.models[Book.name]) {
      console.log('Manually registering Book schema...');
      this.connection.model(Book.name, BookSchema);
    }
    if (!this.connection.models[TitleBook.name]) {
